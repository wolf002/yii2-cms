...
class UserController extends Controller
{
    ...
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'only' => ['login',  'logout', 'confirm', 'reset', 'profile', 'remove', 'online', 'show',
                    'index', 'view', 'update', 'delete', 'rmv', 'multiactive', 'multiblock', 'multidelete', 'create'],
                'rules' => [
                    [
                        'actions' => ['login',  'confirm', 'reset', 'show'],
                        'allow' => true,
                        'roles' => ['?'],
                    ],
                    [
                        'actions' => ['login',  'show', 'logout', 'profile', 'remove', 'online'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                    [
    ...
    /**
     * Создание нового  пользователя в режиме
     * администрирования (по аналогии с личным кабинетом)
     * @param $id - ID пользователя
     * @return string|\yii\web\Response
     * @throws NotFoundHttpException
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            Yii::$app->getSession()->setFlash('success', Yii::t('user', 'Новый пользоввтель создан.'));
            return $this->redirect(['view', 'id' => $model->id]);
        } else {
            return $this->render('@vendor/lowbase/yii2-user/views/user/create', [
                'model' => $model
            ]);
        }
    }
